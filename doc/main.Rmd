...
```{r wkdir, eval=FALSE}
set.seed(2019)
setwd("/Users/zeyu/Downloads/Spring2019-Proj3-spring2019-proj3-grp10") 
...
model_values_xgboost <- expand.grid(max_depth = c(4,6,8),eta =c(0.1,0.3,0.5))
model_labels_xgboost <- paste("XGBoost with depth =",
                              model_values_xgboost$max_depth,
                              "XGBoost with eta =",
                              model_values_xgboost$eta)
##### Tuning paramter: gamma, max_depth, min_child_weight
```
...
if(run.cv){
  ...
  plot(model_values, err_cv[,1], xlab="Interaction Depth", ylab="CV Error",
       main="Cross Validation Error", type="n", ylim=c(0, 0.05))
...
if(run.cv){
  ...
  rownames(err_cv_xgboost_matrix) <- c(0.1,0.3,0.5)
  
  #### plot
  heatmap(err_cv_xgboost_matrix,Colv = NA, Rowv = NA)
}
```
...
```{r}
tm_train_xgboost <- NA
tm_train_xgboost <- system.time(fit_train_xgboost <- train_xgboost(feat_train, label_train, par_best_xgboost))
...
if(run.test){
  ...
  tm_test_xgboost <- system.time(superResolution(test_LR_dir, test_HR_dir, fit_train_xgboost))
}
```
